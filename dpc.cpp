...
//    //DPC::C[7] = new BlackLib::BlackGPIO(BlackLib::I2C_1, BlackLib::input);
    this->analog = new BlackLib::BlackADC(BlackLib::AIN0);
    toDo = false;
    stopAqu = false;
}
...
void DPC_Worker::aquireCounts(void)
{
    ...
    int counts = 4224;
    //QString startValue = DPC_Worker::C[1]->getValue().c_str();
    while(stopAqu == false)
    {
        counts = this->analog->getNumericValue();
        emit this->currentCounts(counts);//Has to be enabled later???
    }
