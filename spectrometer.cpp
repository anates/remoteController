...
Spectrometer::Spectrometer(QMutex *mutex, QWaitCondition *WaitCond)
{
    ...
    connect(stepperControl, &Stepper_Control_Master::CurrentStatus, this, &Spectrometer::StepperStatus);
...
void Spectrometer::StepperStatus(int status)
{
    emit this->stepperStatus(status);
}
...
//    Spectrometer_Control::CountMutex = cmutex;
    ...
    connect(newSpectrometer, &Spectrometer::stepperStatus, this, &Spectrometer_Control::stepperStatus);
...
void Spectrometer_Control::stepperStatus(int status)
{
    emit this->currentStepperStatus(status);
}
